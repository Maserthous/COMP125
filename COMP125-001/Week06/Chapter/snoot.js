...
function removeSelectDefaults() {
   ...
   for (var i = 0; i < emptyBoxes.length; i++) {
      emptyBoxes[i].selectedIndex = -1;//resets the select box
...
function copyBillingAddress() {
   ...
   } else {//user unchecked the same address checkbox and you need to clear the fields
      for (var i = 0; i < billingInputElements.length; i++) {
...
function validateForm(evt) {
   ...
   if (formValidity === true) {
      ...
      document.getElementsByTagName("form")[0].submit();//submits the form
   } else {
